...
Pool.prototype.claim = function(fn) {
  ...
  if (!pool._disposed) {
    ...
    if (pool._connections.length === 0) {
      // Push the claim to the pending operations.
      pool._pending.push({claiming: true, fn: fn});
...
Pool.prototype.query = function(query, values, options, fn) {
  ...
  if (!pool._disposed) {
    ...
    pool._pending.push({claiming: false, query: query, values: values, options: options, fn: fn});
    ...
    while (pool._currentNumberOfConnectionsEstablishing < pool._pending.length) {
...
Pool.prototype._create = function() {
  if(debug) console.log('[my_pool_sql][_create] do we need a new connection: ', pool._currentNumberOfConnections + pool._currentNumberOfConnectionsEstablishing);
  
  // Check if a connection may be established.
  if (pool._currentNumberOfConnections + pool._currentNumberOfConnectionsEstablishing < this.max) {
    // Create a connection.
...
Pool.prototype._update = function() {
  ...
  if(debug) console.log('[my_pool_sql][_update] current pool connections: ', pool._currentNumberOfConnections + pool._currentNumberOfConnectionsEstablishing);
  if(debug) console.log('[my_pool_sql][_update] query queue size: ', pool._pending.length)
  if (pool._pending.length !== 0) {
    // Check if a connection is available.
    if(debug) console.log('[my_pool_sql][_update] connection pool size: ', pool._connections.length);
    if (pool._connections.length !== 0) {
    ...
    // Otherwise a connection may have to be established.
      ...
      // Iterate until sufficient managed _connections are establishing.
      if(debug) console.log('[my_pool_sql][_update] _currentNumberOfConnectionsEstablishing size: ', pool._currentNumberOfConnectionsEstablishing)
      while (pool._currentNumberOfConnectionsEstablishing < pool._pending.length) {
        // Create a managed connection.
        if (!this._create()) {
          break;
  ...
  else if (pool._disposed) {
    // Iterate through each connection.
    for (var i = 0; i < pool._connections.length; i++) {
      // Terminate the connection.
      pool._connections[i]._end();
    }
    // Clear connections.
    pool._connections.length = 0;
  }
};
